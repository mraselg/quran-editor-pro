...
import { BookOpen, ChevronDown, ChevronUp, Download, Eye, EyeOff, FileType, Image as ImageIcon, Layers, Palette, RotateCcw, Sliders, Type, Upload } from "lucide-react";
...
export function Inspector({ page }: { page?: PageData }) {
  ...
  // Properties panel is collapsed by default; expand by clicking the tab
  const [propsPanelOpen, setPropsPanelOpen] = useState(false);
  const editMode = useEditorStore((s) => s.editMode);
  const activeTool = useEditorStore((s) => s.activeTool);

  return (
    <aside className="flex h-full w-[320px] flex-col border-l border-neutral-800 bg-neutral-950 text-neutral-200">
      ...
      {editMode ? (
        ...
        <div className="flex-1 overflow-y-auto p-3 text-xs">
          ...
          {previewTab === "export" && <Placeholder title="Export — PDF/PNG রপ্তানি" />}
        </div>
      )}
    </aside>
  );
